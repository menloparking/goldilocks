...
send_welcome_email(
  ...
  generate_token(length: 32, charset: :alphanumeric),
  {template: "welcome", priority: :high, track_opens: true, track_clicks: true}
)
...
logger.info(
  "Processing batch job ##{job.id}: #{job.items.count} items queued for #{job.queue_name} with priority #{job.priority} at #{Time.current}"
)
