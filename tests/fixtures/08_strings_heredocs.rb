...
"User #{User.includes(:profile).where(active: true,
                                      verified: true).order(created_at: :desc).first.email} logged in from #{request.remote_ip} at #{Time.current.strftime('%Y-%m-%d %H:%M:%S')}"
